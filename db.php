...
class DB{
    ...
    private function a2s($item, $array){
        $tmp = [];
        foreach($array as $key => $arg){
            $tmp[] = "$key = $arg";
        }
        return implode($item, $tmp);
    }
}

function dd($array){ // dd means direct dump;
    echo "<pre>";
    print_r($array);
    echo "</pre>";
}
...
// $status = new DB('school');
// $status->table('status');

// $scores = new DB('school');
// $scores->table('student_scores');

// $students = new DB('school');
